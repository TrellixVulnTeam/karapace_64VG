...
class SchemaRegistryDictDatabase:
    ...
    def log_state(self) -> None:
        if LOG.isEnabledFor(logging.DEBUG):
            ...
            for subject, subject_data in self.subjects.items():
                debug_str += f"\t\t{subject}\n"
                for _, schema_version in subject_data.schemas.items():
    ...
    def get_schema_id_if_exists(
        ...
        if subject in self._hash_to_schema_id_on_subject:
            return self._hash_to_schema_id_on_subject[subject].get(schema.fingerprint(), None)
        return None
    ...
    def find_schema_versions_by_schema_id(self, *, schema_id: SchemaId, include_deleted: bool) -> List[SchemaVersion]:
        ...
        with self.schema_lock_thread:
            for subject, _ in self.subjects.items():
                ...
                for _, schema_version in found_schema_versions.items():
    ...
    def find_subjects(self, *, include_deleted: bool) -> List[Subject]:
        ...
        non_deleted_subjects: List[Subject] = []
        with self.schema_lock_thread:
            for subject, _ in self.subjects.items():
                if self.find_subject_schemas(subject=subject, include_deleted=False):
                    non_deleted_subjects.append(subject)
    ...
    def delete_subject(self, *, subject: Subject, version: ResolvedVersion) -> None:
        with self.schema_lock_thread:
            for _, schema_version in self.subjects[subject].schemas.items():
